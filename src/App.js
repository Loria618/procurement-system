...
/*
2024.5.29 done 在查看订单界面中添加一键展开和折叠所有订单的按钮
在查看订单界面增加生成长截图功能
查看订单选择日期时月份和日期改成中国格式
2024.5.26 第一次打包成apk
done 两个表单在手机页面上显示不全，特别是CompanyDetails中创建订单时无法显示商品名称。
找不到导出JSON文件的路径。
done 删除订单前应该有确认提示框。
日历的边框很奇怪。
需要给按钮添加样式。
...
2024.3.9
MainPage初始显示三家公司（名称可编辑），点击公司之后是新建供货公司和新建表单的选项。并且可以添加新的采购公司。update: 直接创建好就行
done 新建采购表单的日期为当前日期的后一天，并且生成唯一id，方便后续查找。
done MainPage可以显示日历，点击日历中的日期可以显示当天所有采购公司的采购订单（初始折叠，点击可展开），并计算每家采购公司的所要支出的总价和所有采购公司的支出总价。
done 供货公司详情界面增加返回按钮，增加公司信息编辑功能。
2024.3.6
需要修复：
done 新建进货表单时如果没选择模板直接保存也可以成功
